...
| Browser reports CORS failure | Check the requested origin against `AIP_CORS_ORIGINS` in `backend/config.py`. For a deployment, deliberately configure a narrow allow-list rather than broadening it casually. |
...
| Registration fails with a database error | Check write permissions for the directory containing `AIP_DB_PATH`; do not delete the database before taking a backup. |
...
| Old progress disappears | Daily cleanup removes `DONE` progress older than `AIP_CLEANUP_HOURS` (72 by default) and attempts safe in-root screenshot cleanup. |
| `/step` returns an unexpected server error | Confirm the request includes the campaign/profile/task identifiers and a valid base64 PNG, JPEG, or WebP evidence payload. Review the API response and backend traceback without sharing the evidence file. |
...
| Notification errors do not appear in the UI | Provider failures are intentionally best-effort and are not exposed as delivery receipts. Inspect the workflow event row, backend logs, provider-side permissions, and configured credentials. |
